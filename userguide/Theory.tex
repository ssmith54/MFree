...
\end{align*}
which are constant throughout the domain, hence the stress $\sigma^c$ is also constant. Considering the small-strain virtual work expression (equivalent to the except the use of the small strain tensor $\epsilon := sym(\nabla u)$, and Cauchy stress $\sigma$), the virtual work is given by:
\begin{equation}
content...
\end{equation}
\begin{figure} 
