...
func main() {
	...
	// define the nodes
	n1 := domain.New_node(0, 0, 0, 1)
	n2 := domain.New_node(1, 0, 0, 2)
	n3 := domain.New_node(1, 1, 0, 3)
	n4 := domain.New_node(0, 1, 0, 4)
	n5 := domain.New_node(0.5, 0.5, 0, 5)
	n6 := domain.New_node(0.25, 0.25, 0, 6)
	n7 := domain.New_node(0.25, 0.75, 0, 7)
	n8 := domain.New_node(0.75, 0.25, 0, 8)
	n9 := domain.New_node(0.75, 0.75, 0, 9)
	// add nodes to meshfree domain
	nodes = append(nodes, *n1, *n2, *n3, *n4, *n5, *n6, *n7, *n8, *n9)
	numnodes := int(9)

	// set up meshfree domain
	meshfree := domain.NewMeshfree(nodes, numnodes)
